...
<AccordionGroup>
    ...
    <Accordion title="4️⃣ Export data to DuckDB">
        Let's export the data to a local DuckDB database! Create a new `Data Exporter` block and change the type to be `SQL`.

        Next, change the Connection to `DuckDB` and profile to `default`. 
